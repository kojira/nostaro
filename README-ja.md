...
- タグは発行前に検証します。値の無いタグ（`["p"]`）は拒否し、`p` / `e` タグの値は
  **64 文字の hex** のみ受け付けます。`npub1…` を貼ったら「hex に変換せよ」という
  エラーになり、その人だけ黙って欠けたフォローリストが出来上がることはありません。
  （副作用として、NIP-70 の `["-"]` のような要素 1 個のタグは扱えません。）
